...
function getNode(id) {
    ...
    for (var taskId in activeFlows) {
        if (activeFlows.hasOwnProperty(taskId)) {
            node = activeFlows[taskId].getNode(id);
            if (node) {
                return node;
            }
        }
...
function getFlow(id) {
    ...
    if (flow.subflows) {
        var subtaskIds = Object.keys(flow.subflows);
        result.subflows = subtaskIds.map(function(subtaskId) {
            var subflow = clone(flow.subflows[subtaskId]);
